...
Three claims, not equally solid. Cost and context are facts: a full eligibility criterion is 30 tokens with the domain tokenizer against 55 with a general tokenizer of the same vocabulary size, cheaper with more usable context. "Domain always wins" is false: GPT-4's 100k-plus vocabulary is competitive, averaging 3.80 tokens per term against my 4.25 on an 87-term mixed probe set, so the clean win is domain against general at equal vocab (20-45% fewer), not "beats GPT-4". And quality is a flag, not a verdict: fragmentation is a risk to test for rare and exact-match terms, not proof of worse understanding.

The audit panel makes this practical. Paste your own drug names and gene variants and see how GPT-4, GPT-4o, GPT-2, and BERT split them and what that costs; the model view runs the same text through the clinical and general tokenizers at once, so the fragmentation is visible in place. It is a cheap check, and it is why PubMedBERT trained its own vocabulary (https://arxiv.org/abs/2007.15779); the trade-offs are documented in the tokenizer literature (https://arxiv.org/abs/2310.08754).
